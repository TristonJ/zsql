...
fn describe(err: sqlx::Error) -> String {
    match err {
        ...
        sqlx::Error::Io(io_err) => format!("file error: {io_err}"),
...
mod tests {
    ...
    #[test]
    fn io_error_maps_to_connection_error_with_file_prefix() {
        let io_err = std::io::Error::other("unable to open database file");
        let mapped = map_connect_error(sqlx::Error::Io(io_err));
        match mapped {
            CoreError::Connection(msg) => {
                assert_eq!(msg, "file error: unable to open database file");
            }
            other => panic!("expected CoreError::Connection, got {other:?}"),
        }
    }
    ...
    fn configuration_error_maps_to_connection_error() {
        let cfg_err: sqlx::error::BoxDynError = "bad pragma value".into();
    ...
    #[test]
    fn io_error_maps_to_query_error_with_file_prefix() {
        let io_err = std::io::Error::other("unable to read database file");
        let mapped = map_query_error(sqlx::Error::Io(io_err));
        match mapped {
            CoreError::Query(msg) => {
                assert_eq!(msg, "file error: unable to read database file");
            }
            other => panic!("expected CoreError::Query, got {other:?}"),
        }
    }
    ...
    #[test]
    fn io_error_maps_to_introspection_error_with_file_prefix() {
        let io_err = std::io::Error::other("unable to open database file");
