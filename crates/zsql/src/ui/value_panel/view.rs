use gpui::{
    ...
    SharedString, Stateful, Window, actions, div, prelude::*, px, rgb, rgba,
};
...
use super::data::{
    self, BytesMode, JsonLoad, JsonMode, JsonNode, PathSegment, RendererKind, TimestampMode,
    ValuePanelState,
};
use crate::config::ValuePanelConfig;
use crate::ui::format::{self, ValueKind, format_value, format_value_for_clipboard};
use crate::ui::theme;
...
pub struct ValuePanel {
    ...
    /// whenever the panel starts showing a different cell.
    json: Option<ValuePanelJsonCache>,
    /// Our focus handle
...
impl ValuePanel {
    ...
    fn tree_up(&mut self, _: &TreeUp, _window: &mut Window, cx: &mut Context<Self>) {
        self.move_tree_selection(-1, cx);
    }

    fn tree_down(&mut self, _: &TreeDown, _window: &mut Window, cx: &mut Context<Self>) {
        self.move_tree_selection(1, cx);
    }
    ...
    fn tree_collapse(&mut self, _: &TreeCollapse, _window: &mut Window, cx: &mut Context<Self>) {
        let path = self.state.selected_tree_path().to_vec();
        self.state.set_tree_node_expanded(path, false);
        cx.notify();
    }

    fn tree_expand(&mut self, _: &TreeExpand, _window: &mut Window, cx: &mut Context<Self>) {
        let Some(JsonLoad::Parsed(root)) = self.json.as_ref().map(|c| &c.load) else {
            return;
        };
        let path = self.state.selected_tree_path().to_vec();
